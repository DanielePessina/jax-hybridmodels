...
def _trainable_param_count(mask, predictor) -> int:
    total = 0
    for m, p in zip(
        jtu.tree_leaves(mask), jtu.tree_leaves(predictor), strict=True
    ):
        if not bool(m):
            continue
        if hasattr(p, "shape"):
...
def main() -> None:
    bp = BoundedPredictor(
        ...
        in_scaler=BoundScaler(
            bounds=((20.0, 40.0), (0.05, 0.30)), transform="sigmoid"
        ),
        inner=MLPPredictor(
            in_size=2,
            out_size=2,
