...
VitePress documentation site for the `jax-hybridmodels` distribution and its
...
The Pages workflow runs on relevant pushes to `main` and can also be started
with `workflow_dispatch`. Generated API pages must be regenerated and
committed locally; CI checks that they are synchronized before deploying.
