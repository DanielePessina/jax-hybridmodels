...
- Verifies subagent output: tests green, ruff clean, mypy clean, behaviour parity vs source `hybridcrystals` where the phase calls for it.
...
<paste failing pytest / ruff / mypy output>
...
| 0.4 | `uv add --dev pytest pytest-cov ruff mypy` | O | `uv run pytest --version` |
...
| 0.6 | Create empty module files per SPEC §3 (`data.py`, `solver.py`, `losses.py`, `trainable.py`, `rng.py`, `prediction.py`, `serialise.py`, `predictors/{__init__,base,mlp,kan,neural_npoly}.py`, `training/{__init__,optax,evosax}.py`, `ui/{__init__,base,optax,evosax}.py`) | O | `find src/hybridmodels -name '*.py'` |
...
| 0.10 | Configure mypy in `pyproject.toml` (strict-ish; tolerant of equinox quirks via `ignore_missing_imports` for jaxkan) | O | `uv run mypy src` passes |
...
| 3.2 | Subagent: write `tests/test_predictors_base.py` red — `CovariateSelector` ordering, `BoundScaler` sigmoid bidirection (`from_latent(to_latent(x)) ≈ x` within bounds), `BoundedPredictor` composition, `RatePair` stacking, `reinitialize_with_key` re-inits inexact-float leaves | S | red |
| 3.3 | Subagent: implement `src/hybridmodels/predictors/base.py` to green | S | green |
| 3.4 | `__init__.py` exports | O | import smoke |
| 3.5 | ruff + mypy | O | clean |
| 3.6 | Commit `feat(predictors): base — Predictor, BoundedPredictor, RatePair, BoundScaler` + push | O | clean |
...
| 4.4 | ruff + mypy + commit + push | O | clean |
...
| 5.5 | ruff + mypy + commit + push | O | clean |
...
| 6.3 | ruff + mypy + commit + push | O | clean |
...
| 7.3 | ruff + mypy + commit + push | O | clean |
...
| 8.3 | ruff + mypy + commit + push | O | clean |
...
| 9.4 | ruff + mypy + commit + push | O | clean |

---

### Phase 10 — RichTrainingUI (subagent — visual)

SPEC §8 step 11. **No new tests beyond the spy** — Rich rendering is verified by eye in Phase 15.
...
| 10.3 | ruff + mypy + commit + push | O | clean |
...
| 11.3 | ruff + mypy + commit + push | O | clean |
...
| 13.4 | ruff + mypy + commit + push | O | clean |

---

### Phase 14 — NeuralNPolynomial (subagent)

SPEC §8 step 15.

| # | Task | Who | Verify |
|---|---|---|---|
| 14.1 | Subagent: extend `tests/test_predictors_serialise.py` parametrise list with `NeuralNPolynomial(coeff_net=MLPPredictor(...), exponents=(0.0, 1.0, 2.0))` | S | red |
| 14.2 | Subagent: implement `src/hybridmodels/predictors/neural_npoly.py` — pure composition `(coeff_net, exponents)` | S | green |
| 14.3 | ruff + mypy + commit + push | O | clean |

---

### Phase 15 — Crystallisation example (subagent — verification gate)

SPEC §8 step 16. **The integration gate before serialisation ships.**

| # | Task | Who | Verify |
|---|---|---|---|
| 15.1 | Subagent: port `examples/crystallisation/loader_excel.py` from source — isolated, `openpyxl` extras | S | smoke load |
| 15.2 | Subagent: write `examples/crystallisation/ode.py` — moments + concentration `simulate_fn` (mandatory signature) | S | smoke run |
| 15.3 | Subagent: write `examples/crystallisation/kinetic_predictor.py` — CNT nucleation + power-law growth as `BoundedPredictor`s | S | shapes |
| 15.4 | Subagent: write `examples/crystallisation/mlp_predictor.py` — `RatePair(MLP, MLP)` with appropriate `BoundedPredictor` wrapping | S | shapes |
| 15.5 | Subagent: write `examples/crystallisation/train_optax.py` — end-to-end script reproducing `thesis_training/sharedgrowth.py` | S | runs |
| 15.6 | **Verification gate**: orchestrator runs both new and source scripts on the same Excel data; asserts final loss within `rtol=1e-2` | O | numerical |
| 15.7 | Subagent: write `examples/crystallisation/train_evosax_kinetic.py` — same data, evosax search over kinetic-only `RatePair` | S | runs |
| 15.8 | Commit + push | O | clean |

---

### Phase 16 — Serialisation (subagent — last shipped per R-A5)

SPEC §8 step 17.

| # | Task | Who | Verify |
|---|---|---|---|
| 16.1 | Subagent: write `tests/test_serialise.py` red — `save_predictor`/`load_predictor` round-trip, `save_run`/`load_run` directory-shape contract per CONTEXT.md | S | red |
| 16.2 | Subagent: implement `src/hybridmodels/serialise.py` — four functions, `metadata.json` shape | S | green |
| 16.3 | **Verify against Phase 15**: save a trained predictor, reload, re-run prediction, assert bit-exact agreement | O | numerical |
| 16.4 | ruff + mypy + commit + push | O | clean |

---

### Phase 17 — Pendulum example (subagent — domain-agnostic gate)

SPEC §8 step 18. **Final v1 deliverable.**

| # | Task | Who | Verify |
|---|---|---|---|
| 17.1 | Subagent: write `examples/pendulum/train.py` — pendulum `simulate_fn` (closed-form `θ̈ = -(g/L)sin(θ)`), MLP predictor for unknown `g/L`, train via `train_with_optax` to recover the constant | S | runs |
| 17.2 | **Domain-agnostic gate**: orchestrator runs `rg -i "crystal\|moment\|mu0\|d43" src/hybridmodels/`; asserts no matches | O | grep clean |
| 17.3 | Final commit + tag `v0.1.0` + push tags | O | tag pushed |
...
<paste failing pytest / ruff / mypy output verbatim>
...
| 15 | Final crystallisation training loss vs source | `rtol=1e-2` |
| 16 | Reloaded predictor predictions vs in-memory | bit-exact |
| 17 | Recovered `g/L` vs ground truth | `rtol=1e-2` |

### Domain-agnostic gate (Phase 17)
