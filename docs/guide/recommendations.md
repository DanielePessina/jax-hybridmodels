...
Practical guidance from the canonical examples. The framework enforces none of it, and all of it will save you a debugging session.
...
A common failure mode is bounds that are too wide. They push the centre into a stiff regime, and the integrator hits `max_steps` before training even starts. The crystallisation example documents this explicitly:
...
- `diffrax.Heun()`: cheap and low-order; reasonable for very fast feasibility tests.
...
If your trainable parameter set is small (~4-10 parameters), kinetic-parameter style, or known to be multi-modal, `train_with_evosax` is the right tool. Run it first, then polish with `train_with_optax`. Both accept the same `trainable=` mask.
...
A naive `jnp.where(divisor > eps, num / divisor, 0.0)` still computes `num / divisor` on the masked-out branch. That produces `inf` / `nan` whose gradient flows back through `jnp.where` and poisons the loss. Wrap the divisor:
