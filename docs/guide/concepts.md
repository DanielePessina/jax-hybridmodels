...
This page is the package's vocabulary. Every term shows up in API docstrings, error messages, and example comments. Knowing what each one is, and where its boundary sits, makes the rest of the docs easy to navigate.
...
Concrete subclasses are final. There is no inheritance chain; you compose. To write your own predictor, subclass [`Predictor`](/api/predictors#predictor) directly with whatever fields you need (the [pendulum example](/examples/pendulum) does exactly this with a one-leaf `OmegaPredictor`).
...
It has two methods, [`to_latent`](/api/predictors#boundscaler) and `from_latent`, so the conversion is bidirectional. The transform is sigmoid only in v1 (tanh deferred). The optional `temperature` field is a leaf, frozen by convention in every example via [`freeze_modules_of_type`](/api/trainable#freeze_modules_of_type).
...
A pure callable mapping the full-state trajectory `[T, S]` to the observed channels `[T, D]`. Crystallisation: `[mu0, mu1, mu2, mu3, mu4, conc] → [conc, d43]`. Stored as a static field on [`Dataset`](/api/data#dataset), and applied by [`predict_bucket`](/api/prediction#predict_bucket) before any loss computation.
...
Three terms that look similar but differ in scope:
...
The first argument of `simulate_fn` is a `PyTree[eqx.Module]`. The framework is runtime-permissive: any pytree of leaves works (single Module, tuple, dict, NamedTuple, custom container). The *convention* is to wrap in a tuple, so a single-predictor case is `(predictor,)` and the surrounding code never branches on container type.
...
A PyTree of booleans matching the predictors pytree's structure. The training loop calls `eqx.partition(predictors, mask)` once at start, optimises only the `True` leaves, and re-combines. The default predicate is `eqx.is_inexact_array`, making every float array trainable.
