...
features:
  - title: Composition over inheritance
    details: A model is a triple of a predictors PyTree, a user-supplied simulate_fn, and a SolverConfig. The framework runs the JAX plumbing (vectorisation across experiments, JIT, gradients through the integrator); the user supplies the physics. There is no Model base class to subclass.
  ...
  - title: Irregular time-series
    details: Each observation channel carries its own timestamps. make_dataset forms per-experiment union grids, derives observation masks automatically, and groups experiments by grid length into buckets. Each bucket is a JIT cache key, so there is one compilation per bucket shape, then full JAX speed across every step.
