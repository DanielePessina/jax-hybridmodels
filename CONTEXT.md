...
- Shared tournament only. Enabled implicitly when `tournament_steps > 0 AND tournament_attempts > 1`. Serial candidate evaluation that *reuses the main loop's compiled `make_step` and `apply_update`* (no extra JIT compile cost). Each candidate is a fresh re-init trained for `tournament_steps` at `tournament_lr`, then scored on the data term alone with a forward-only pass; the lowest score wins. Scoring on the data term rather than the combined objective stops a candidate winning by drifting somewhere the penalty likes. On per-attempt failure (diffrax error / non-finite loss), drop and try a fresh RNG; if all fail, fall back to the original predictors with a `RuntimeWarning`.
