...
name. The names used internally are ``"init"``, ``"tournament"``,
``"phase_{i}"``, ``"evosax_init"``, and ``"evosax_ask_{gen}"``.
...
means adding a ``"warmup"`` fold leaves ``"phase_0"`` exactly where it
was, and old runs still reproduce bit for bit.
